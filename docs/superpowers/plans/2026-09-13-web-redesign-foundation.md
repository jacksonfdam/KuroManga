...
- **A Tailwind class is only real if the built CSS contains it.** Three separate times on this branch a class has looked plausible, satisfied TypeScript, passed the build and generated nothing: a name built by interpolation, which the scanner never sees; `font-label-md`, a scale name defined only under `fontSize`; and `bg-secondary/12`, an opacity modifier off Tailwind's scale, where the bracket form `/[0.12]` was needed. They fail identically and silently. So verification for any task touching the interface includes grepping `frontend/dist/` for the classes it introduced — the source is what lies, the build output is what tells the truth — and the report says what was grepped and found.
